...
import type { CategoryKey, MockShiftLog, VerifiedLog } from "./types";

export type { CategoryKey, MockShiftLog, VerifiedLog };
...
export function getVerifiedHoursFromDb(logs: VerifiedLog[]): number {
  return logs
    .filter((log) => log.verifiedAt !== null)
    .reduce((sum, log) => sum + log.hours, 0);
}

export function getPendingHoursFromDb(logs: VerifiedLog[]): number {
  return logs
    .filter((log) => log.verifiedAt === null)
    .reduce((sum, log) => sum + log.hours, 0);
}

export function getVerifiedHours(logs: MockShiftLog[]): number {
  return logs
