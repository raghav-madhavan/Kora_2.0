...
export const OPEN_MODERATOR_PALETTE_EVENT = "kora:moderator-palette";

/** Opens the moderator command palette from anywhere (e.g. the topbar trigger). */
export function openModeratorCommandPalette() {
  window.dispatchEvent(new CustomEvent(OPEN_MODERATOR_PALETTE_EVENT));
}
...
const destinations: Destination[] = [
  { icon: CircleGauge, label: "Dashboard", href: "/moderator", key: "d", keywords: "home overview stats" },
  { icon: BadgeCheck, label: "Verifications", href: "/moderator/verifications", key: "v", keywords: "claims approve reject pending flagged" },
  { icon: CalendarDays, label: "Shifts", href: "/moderator/shifts", key: "s", keywords: "calendar qr check in display" },
  { icon: MessageSquare, label: "Messages", href: "/moderator/messages", key: "m", keywords: "inbox threads students" },
  { icon: User, label: "Profile", href: "/moderator/profile", key: "p", keywords: "account org settings" },
];
...
export function CommandPalette() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_MODERATOR_PALETTE_EVENT, onOpenEvent);
    return () => {
      window.removeEventListener("keydown", onKeyDown);
      window.removeEventListener(OPEN_MODERATOR_PALETTE_EVENT, onOpenEvent);
      clearChord();
    };
  }, [open, router]);
  ...
  const activate = (index: number) => {
    if (index === searchRowIndex) {
      go(`/moderator/search?q=${encodeURIComponent(trimmed)}`);
      return;
    }
      ...
      <div className="w-full max-w-[560px] overflow-hidden rounded-card bg-surface shadow-raised">
        <div className="flex items-center gap-3 border-b border-black/5 px-5 py-4">
          ...
          <input
            ...
            placeholder="Jump to a page or search the org portal…"
